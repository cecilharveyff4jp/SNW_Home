...
export default function Home() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      bearImg.src = `${basePath}/bear-img-${numStr}.png`;
      const index = i - 1;
      bearImg.onload = () => {
    ...
    trapImg.src = `${basePath}/bear-trap-img.png`;
    trapImg.onload = () => {
      ...
      soldierAnimations.forEach((anim) => {
        ...
        if (progress > 0.28 && progress < 1.0) {
          ...
          if (isAttacking) {
            ...
            // 攻撃の火花（小さく、薄く）
            const sparkCount = 6; // 12→6に減少
            for (let i = 0; i < sparkCount; i++) {
              const angle = (i / sparkCount) * Math.PI * 2 + time / 100;
              const distance = attackPhase * size * 2.5;
              const sparkX = Math.cos(angle) * distance;
              const sparkY = Math.sin(angle) * distance;
              const sparkSize = (1 - attackPhase) * size * 0.15; // 0.3→0.15に縮小
              
              ctx.fillStyle = `rgba(255,${200 - attackPhase * 150},0,${(1 - attackPhase) * 0.4})`; // 透明度を0.4倍に
              ctx.shadowColor = "rgba(255,150,0,0.5)"; // shadowBlurを薄く
              ctx.shadowBlur = 8; // 15→8に
              ctx.beginPath();
